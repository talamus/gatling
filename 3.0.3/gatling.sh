...
# Slightly edited 2019 by Tero Niemi (https://github.com/talamus)
#
...
COMPILER_ARGS_FROM_FILE=`cat $GATLING_CONF/compiler_args.txt` 2> /dev/null
GATLING_ARGS_FROM_FILE=`cat $GATLING_CONF/gatling_args.txt` 2> /dev/null

# Run the compiler
echo Arguments for Compiler are: $COMPILER_ARGS $COMPILER_ARGS_FROM_FILE $EXTRA_COMPILER_OPTIONS $@
"$JAVA" $COMPILER_OPTS -cp "$COMPILER_CLASSPATH" io.gatling.compiler.ZincCompiler $COMPILER_ARGS $COMPILER_ARGS_FROM_FILE $EXTRA_COMPILER_OPTIONS "$@" 2> /dev/null
# Run Gatling
echo Arguments for Gatling are: $GATLING_ARGS $GATLING_ARGS_FROM_FILE $@
"$JAVA" $DEFAULT_JAVA_OPTS $JAVA_OPTS -cp "$GATLING_CLASSPATH" io.gatling.app.Gatling $GATLING_ARGS $GATLING_ARGS_FROM_FILE "$@"
